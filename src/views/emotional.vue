...
<template>
  <div>
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column  label="情绪评分" width="230">
        <template #default="scope">
      ...
      <div class="detail-section">
       ...
       <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="主要情绪">
          <el-tag :type="getEmotionTagType(currentDetail.domainEmotion)">{{currentDetail.domainEmotion || '-'}}</el-tag>
        </el-descriptions-item>
      ...
      <div class="detail-section">
       ...
       <div class="ai-analysis-result">
        ...
        <el-descriptions-item label="情绪强度">
          <el-progress :percentage="aiData.emotionScore" :color="getEmotionScoreColor(aiData.emotionScore)" :stroke-width="10" />
        </el-descriptions-item>
       ...
       <div class="ai-suggestion-section">
          ...
          <div class="suggestion-content">
            <p>{{aiData.suggestion || '无'}}</p>
          </div>
       </div>
       <div class="ai-risk-section">
          <h5>风险描述</h5>
          <div class="risk-content">
            <p>{{aiData.riskDescription || '无'}}</p>
          </div>
       </div>
...
const viewSessionDetail = (row) => {
  ...
  if(row.aiEmotionAnalysis){
    try {
      parsed = JSON.parse(row.aiEmotionAnalysis)
    } catch (e) {
      parsed = {}
    }
  }
  // 改善建议兜底：统一成数组，避免 v-for 遍历字符串/null 出现异常渲染
  if (typeof parsed.improvementSuggestion === 'string') {
    parsed.improvementSuggestion = [parsed.improvementSuggestion]
  } else if (!Array.isArray(parsed.improvementSuggestion)) {
